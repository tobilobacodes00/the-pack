...
from app.engine.roster import scout_spec
...
def rehearse(team: list[dict], strategy: str, depth: str = "standard") -> dict:
    """Estimate a hunt's cost (USD) and time (s) for this team + strategy, without running it.

    Depth changes the SCOUT tier (deep → plus+thinking) and applies a heuristic surcharge for the
    longer prompts/outputs the deeper counts imply. An honest ballpark for the card's preview row."""
    scouts = max(1, min(5, _scout_count(team)))
    rounds = 2 if strategy == "deep_dive" else 1
    calls: list[str] = []
    scout_tier = scout_spec(depth)[0]  # v3: deep scouts run on plus, not flash
    calls += [scout_tier] * (scouts * rounds)  # each scout summarizes its angle, per round
    ...
    est_cost = round(sum(pricing.estimate(t) for t in calls) * mult, 4)
    # Scouts run in parallel, so wall-clock ≈ rounds*(search+read) + merge + critique + draft.
    est_time = int((rounds * 30 + 20 + 15 + 20) * mult)
    return {
