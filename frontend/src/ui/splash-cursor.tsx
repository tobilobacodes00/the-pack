...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!container) return;
    // Respect reduced motion: no sim, no GL context, no listeners.
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

    // Fresh canvas per mount: teardown calls loseContext(), and a reused canvas element
    // would hand back the same dead context on remount (StrictMode double-mounts in dev).
    ...
    function scaleByPixelRatio(input: number) {
      // Smoke is soft by nature — capping the backing store at 1.25x DPR is visually
      // indistinguishable and roughly halves the fill cost on 2x displays.
    ...
    let rafId = 0;
    let running = false;
    // The sim parks itself shortly after the last pointer event — by then the dye has
    // fully dissipated (DENSITY_DISSIPATION >> 1 fades it in under a second), so idle
    // GPU cost is zero. wake() restarts it on the next pointer event.
    let idleAt = 0;
    const IDLE_MS = 2500;
    ...
    // sim loop against a detached canvas — the site got slower with every transition.
    return () => {
      running = false;
      idleAt = 0;
