...
import { ROLE_DESC, ROLE_COLOR } from './roles'
 ...
 * taken verbatim from the "idle state" design. Shared by the canvas node and the
 * roster so an inactive pack reads identically in both places. `size` only scales
 * the render; the 80-unit viewBox (centred on the glyph) is preserved.
 */
export function IdleGlyph({ role, size = SIZE, tone = 'idle' }: { role: string; size?: number; tone?: AgentTone }) {
  ...
  // Keep the disc dark, and just light up the rings and the glyph itself.
  const discFill = '#1A1A1A'
  const glyphColor = idle ? color : color
  
  return (
    // `glyphColor` drives the glyph via currentColor; rings + faint halo pick up the role colour once active.
    <svg width={size} height={size} viewBox={viewBox} fill="none" style={{ display: 'block', color: glyphColor }}>
