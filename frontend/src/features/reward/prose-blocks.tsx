...
/** The brief body: one paragraph per block. Cited paragraphs hover violet + open provenance. */
  ...
  return (
    <div className="flex flex-col gap-5">
      {blocks.map((b, i) => {
        ...
        if (text.startsWith('## ')) {
          return (
            <h2 key={i} className="mt-2 max-w-[600px] text-[17px] font-semibold text-[#EDEDED]">
              {text.slice(3).trim()}
            </h2>
          )
        }
          ...
          <div key={i} className="flex items-start justify-between gap-8">
            {ids.length ? (
              <p
                ...
                className="-mx-2 min-w-0 max-w-[600px] flex-1 cursor-pointer whitespace-pre-wrap rounded-md px-2 py-0.5 text-[15px] leading-[1.75] text-[#C7C7C7] transition-colors hover:bg-[rgba(139,92,246,0.14)]"
              >
                {text}
              </p>
            ) : (
              <p className="min-w-0 max-w-[600px] flex-1 whitespace-pre-wrap text-[15px] leading-[1.75] text-[#C7C7C7]">
                {text}
              </p>
            )}
